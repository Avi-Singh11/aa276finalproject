...
def load(ckpt_path: str):
    ...
    num_hidden_layers = (
        sum(1 for k in state if k.startswith("net.net.") and k.endswith(".weight")) - 2
    )
    cfg = {
        **CFG,
...
def plot_slice(ax, V_grid, x_vals, y_vals, xlabel, ylabel, title, dyn):
    ...
    im = ax.contourf(x_vals, y_vals, V, levels=40, cmap="RdYlGn", vmin=-vmax, vmax=vmax)
    ax.contour(x_vals, y_vals, V, levels=[0.0], colors="black", linewidths=2.0)
...
def main(ckpt_path: str, t: float, out_dir: str):
    ...
    plot_slice(
        ...
        "x_slosh (m)",
        "y_slosh (m)",
        "Slosh position  (vx=vy=0, arm at home)",
        dyn,
    )
    # Draw slosh radius limit
    theta_c = np.linspace(0, 2 * np.pi, 200)
    axes[0, 0].plot(
        dyn.slosh_rad_max * np.cos(theta_c),
        dyn.slosh_rad_max * np.sin(theta_c),
        "b--",
        lw=1.5,
        label=f"slosh_rad_max={dyn.slosh_rad_max:.4f}m",
    )
    ...
    plot_slice(
        ...
        "vx_slosh (m/s)",
        "vy_slosh (m/s)",
        "Slosh velocity  (x=y=0, arm at home)",
        dyn,
    )
    ...
    plot_slice(
        ...
        "x_slosh (m)",
        "vx_slosh (m/s)",
        "Slosh phase portrait  (y=vy=0, arm at home)",
        dyn,
    )
    ...
    plot_slice(
        ...
        "q1 (rad)",
        "q2 (rad)",
        "Joint angle plane  (slosh at rest)",
        dyn,
    )
    ...
    print(
        f"  (slosh_rad_max={dyn.slosh_rad_max:.4f}m  training range ±{slosh_scale:.4f}m)"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt",
        default="brt_model_final2/model_final2.pth",
    ...
    parser.add_argument(
        "--out", default="brt_visualizations", help="Output directory for plots"
    )
