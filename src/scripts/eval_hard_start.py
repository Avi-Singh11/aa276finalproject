"""
Comprehensive safety evaluation: PPO baseline vs PPO + BRT safety filter.

Evaluates on:
  - Normal starts: drawn from the standard training distribution
  - Hard starts:   low cup height (el∈[2°,12°]) + random joint velocity (±2 rad/s)

Metrics reported:
  - Failure rate (overall + breakdown: ground / spill / obstacle / joint)
  - Completion rate (|cup - goal| < 0.1 m)
  - Truncation rate (time limit reached)
  - Mean episode length (steps)
  - Mean peak slosh displacement
  - BRT intervention rate (steps where filter overrode PPO)

Usage:
    # Normal starts only, PPO baseline:
    python -m src.scripts.eval_hard_start --mode normal

    # Hard starts only, PPO baseline:
    python -m src.scripts.eval_hard_start --mode hard

    # Both modes, with BRT filter:
    python -m src.scripts.eval_hard_start --mode both --use_filter

    # Full comparison for paper (recommended):
    python -m src.scripts.eval_hard_start --mode both --use_filter --n_seeds 50
"""
...
class LowStartEnv(CoffeeArmEnv):
    ...
    Cup starts at el ∈ [EL_LOW_DEG, EL_HIGH_DEG] (z ≈ 2–14 cm above floor),
    with joint velocities drawn from Uniform([-VEL_SCALE, VEL_SCALE] rad/s).
    Training used el ∈ [0°, 30°] and zero initial velocity, so both stressors
    are out-of-distribution for the PPO policy.
    """
    EL_LOW_DEG  = 2.0
    EL_HIGH_DEG = 12.0
    VEL_SCALE   = 2.0  # rad/s
...
CKPT_BASE   = os.path.join(PROJECT_ROOT, 'checkpoints', 'same_sector_v1')
FINAL_MODEL = os.path.join(PROJECT_ROOT, 'ppo_same_sector_v1_final.zip')
FINAL_VN    = os.path.join(PROJECT_ROOT, 'ppo_same_sector_v1_vecnormalize.pkl')


def load_ppo(ckpt: str = 'final', env_cls=CoffeeArmEnv):
    env_kwargs = dict(u_max=15.0, T=10.0, dt=0.01)
    make_env   = lambda: env_cls(**env_kwargs)
    ...
    if ckpt == 'final':
        ...
        steps   = int(ckpt)
        model   = PPO.load(f'{CKPT_BASE}/ppo_same_sector_v1_{steps}_steps.zip')
        vec_env = VecNormalize.load(
            f'{CKPT_BASE}/ppo_same_sector_v1_vecnormalize_{steps}_steps.pkl',
...
def record_rollout(model, vec_env, seed: int, brt_bundle=None, deterministic=True):
    """Run one episode and return per-step data + outcome flags.

    Returns a dict with arrays (one entry per step) and scalar outcomes.
    Failure types are tracked separately (below_ground, spill, obstacle, joint).
    """
    ...
    while not done:
        ppo_action, _ = model.predict(obs, deterministic=deterministic)
        u_nom = ppo_action[0].copy()

        if brt_nn is not None:
            u_exe, did_intervene = brt_safety_filter(
                brt_nn, brt_dyn, raw_env.state.copy(), u_nom, t=BRT_CFG['tMax']
            )
        ...
        obs, _, done_arr, info_arr = vec_env.step(ppo_action)
        info = info_arr[0]
        ...
        actions.append(ppo_action[0].copy())
    ...
    truncated = bool(not failed and not completed)
...
def evaluate_batch(model, vec_env, seeds, brt_bundle=None, label='PPO'):
    results = []
    for seed in seeds:
        ep = record_rollout(model, vec_env, seed, brt_bundle=brt_bundle)
        results.append(ep)
        if ep['failed']:
...
def plot_results(
    ...
    out_path: str = 'figures/eval_comparison.png',
):
    """
    Paper-quality 6-panel figure.
      A: Outcome rates bar chart (normal + hard starts, PPO vs PPO+BRT)
      B: Failure type breakdown (hard starts)
      C: Cup height traces – hard starts, PPO baseline
      D: Cup height traces – hard starts, PPO + BRT
      E: Distance to goal traces – hard starts
      F: BRT intervention rate per episode
    """
    ...
    if norm_bl is not None:
        s = summarise(norm_bl, verbose=False)
        conditions.append(('Normal / PPO', BLUE,  s))
    if norm_ft is not None:
        s = summarise(norm_ft, verbose=False)
        conditions.append(('Normal / PPO+BRT', GREEN, s))
    if hard_bl is not None:
        s = summarise(hard_bl, verbose=False)
        conditions.append(('Hard / PPO', 'darkorange', s))
    if hard_ft is not None:
        s = summarise(hard_ft, verbose=False)
        conditions.append(('Hard / PPO+BRT', 'mediumorchid', s))
    ...
    if hard_bl is not None or hard_ft is not None:
        ...
        ft_conds = []
        if hard_bl is not None:
            ft_conds.append(('PPO',      'darkorange', summarise(hard_bl, verbose=False)))
        if hard_ft is not None:
            ft_conds.append(('PPO+BRT',  'mediumorchid', summarise(hard_ft, verbose=False)))
    ...
    _draw_traces(ax_zbl, hard_bl, BLUE,  '(C)  Hard starts — PPO baseline (cup height)')
    _draw_traces(ax_zft, hard_ft, GREEN, '(D)  Hard starts — PPO + BRT filter (cup height)')

    # ── Panel E: distance to goal ─────────────────────────────────────────────
    plotted = False
    for eps, col, lbl in [
        (hard_bl, BLUE,  'PPO baseline'),
        (hard_ft, GREEN, 'PPO + BRT'),
    ]:
        if eps is None:
            continue
        for ep in eps:
    ...
    if plotted:
        handles = [
            Line2D([0], [0], color=BLUE,  lw=1.5, label='PPO baseline'),
            Line2D([0], [0], color=GREEN, lw=1.5, label='PPO + BRT filter'),
    ...
    if hard_ft is not None and any(r['brt_overrides'].any() for r in hard_ft):
        ...
        ax_ovrd.set_title('(F)  BRT filter activity', fontsize=9, fontweight='bold',
                          color=GRAY)
    ...
    if hard_bl is not None:
        s = summarise(hard_bl, verbose=False)
        parts.append(f'PPO hard-start failure {s["fail_rate"]*100:.0f}%')
    if hard_ft is not None:
        s = summarise(hard_ft, verbose=False)
        parts.append(f'PPO+BRT hard-start failure {s["fail_rate"]*100:.0f}%')
    suptitle = ('Safety evaluation: PPO vs PPO + BRT safety filter\n'
                + '  |  '.join(parts))
...
def main():
    ...
    parser.add_argument('--mode', choices=['normal', 'hard', 'both'], default='hard',
                        help='Which start distribution to evaluate')
    parser.add_argument('--use_filter', action='store_true',
                        help='Also run PPO + BRT filter comparison')
    parser.add_argument('--ckpt', default='final',
                        help='"final" or step count e.g. "2000000"')
    parser.add_argument('--brt_model', default=None,
                        help='Override BRT model path')
    parser.add_argument('--n_seeds', type=int, default=50)
    parser.add_argument('--out', default='figures/eval_comparison.png')
    ...
    if args.mode in ('normal', 'both'):
        print(f'\n{"="*56}')
        print(f'NORMAL STARTS  (n={args.n_seeds}, ckpt={args.ckpt})')
        print(f'{"="*56}')

        model, vec_env = load_ppo(args.ckpt, env_cls=CoffeeArmEnv)
        print(f'\nPPO baseline:')
        norm_bl = evaluate_batch(model, vec_env, seeds, brt_bundle=None, label='PPO')
        summarise(norm_bl, label='Normal starts — PPO baseline')

        if brt_bundle is not None:
            _, vec_env_f = load_ppo(args.ckpt, env_cls=CoffeeArmEnv)
            print(f'\nPPO + BRT filter:')
            norm_ft = evaluate_batch(model, vec_env_f, seeds, brt_bundle=brt_bundle,
                                     label='PPO+BRT')
            summarise(norm_ft, label='Normal starts — PPO + BRT filter')
    ...
    if args.mode in ('hard', 'both'):
        ...
        model_h, vec_env_h = load_ppo(args.ckpt, env_cls=LowStartEnv)
        print(f'\nPPO baseline:')
        hard_bl = evaluate_batch(model_h, vec_env_h, seeds, brt_bundle=None, label='PPO')
        summarise(hard_bl, label='Hard starts — PPO baseline')

        if brt_bundle is not None:
            _, vec_env_hf = load_ppo(args.ckpt, env_cls=LowStartEnv)
            print(f'\nPPO + BRT filter:')
            hard_ft = evaluate_batch(model_h, vec_env_hf, seeds, brt_bundle=brt_bundle,
                                     label='PPO+BRT')
            summarise(hard_ft, label='Hard starts — PPO + BRT filter')
