...
from dynamics.dynamics import ARM_LINK_RADIUS, CoffeeArmDynamics
...
def _boundary_fn_torch(dynamics, x: torch.Tensor) -> torch.Tensor:
    """Torch equivalent of CoffeeArmDynamics.boundary_fn for physical states."""
    q = x[..., :3]
    l1, l2, l3 = (
        torch.as_tensor(dynamics.L[i], dtype=x.dtype, device=x.device)
        for i in range(3)
    )
    l_total = l1 + l2 + l3
    ...
    link3_points = p2.unsqueeze(-2) + t_sc.view(1, -1, 1) * (
        p3 - p2
    ).unsqueeze(-2)
    ...
    margins = torch.stack([
        ...
        (dynamics.slosh_rad_max - slosh_disp)
        / (10.0 * dynamics.slosh_rad_max),
...
def _targeted_boundary_states(dynamics, n: int) -> np.ndarray:
    ...
    n_joint = max(
        1,
        n - len(dynamics.obstacles) * n_per_obs
        - n_slosh - n_ground - n_selfc,
    )
...
def train_brt(cfg=CFG, model=None, dynamics=None):
    ...
    while static_collected < N_STATIC_POOL:
        ...
        # Only zero-displacement, zero-velocity slosh states are equilibria.
        # A displaced pendulum accelerates under gravity even when v_slosh=0.
        cands = zero_slosh
    ...
    for epoch in tqdm(range(cfg['epochs']), desc='BRT training'):
        ...
        scale = torch.as_tensor(
            dynamics.state_scale, dtype=z_pde.dtype, device=DEVICE
        )
        ...
        dirichlet_mask = torch.zeros(
            cfg['numpoints'], dtype=torch.bool, device=DEVICE
        )
