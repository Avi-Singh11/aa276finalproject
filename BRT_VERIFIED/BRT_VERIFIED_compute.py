"""Verified BRT training entry point for the 10D coffee arm model."""
...
from BRT_VERIFIED_dynamics import BRTVerifiedCoffeeArmDynamics
from BRT_VERIFIED_preflight import run_preflight
...
CFG_FAST = {
    **CFG,
    ...
    'counter_end':      25_000,   
    'save_dir':         'BRT_VERIFIED_model_fast',
}
...
CFG_BENCHMARK = {
    **CFG,
    ...
    'counter_end':        20,
    'save_dir':           'BRT_VERIFIED_benchmark',
    'log_every':          5,
}
...
CFG_PILOT = {
    **CFG,
    ...
    'save_dir':           'BRT_VERIFIED_model_pilot',
    'log_every':          250,
}
...
def train_brt(cfg=CFG, model=None, dynamics=None):
    ...
    if dynamics is None:
        dynamics = BRTVerifiedCoffeeArmDynamics()
    ...
    for epoch in tqdm(range(cfg['epochs']), desc='BRT training'):
        ...
        if (epoch + 1) % log_every == 0:
            ...
            tqdm.write(f"  [{epoch+1:>6}] {loss_parts}  V_std={v_std:.5f}")
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        dyn = BRTVerifiedCoffeeArmDynamics()
