...
# Arm radius used for self-collision checks.
...
class CoffeeArmDynamics:
    ...
    def coord_to_input(self, coord):
        ...
        scale = torch.as_tensor(
            self.state_scale, dtype=torch.float32, device=coord.device
        )
    ...
    def input_to_coord(self, inp):
        ...
        z = inp[..., 1:]
        scale = torch.as_tensor(
            self.state_scale, dtype=torch.float32, device=inp.device
        )
    ...
    def failure_margin(self, state):
        ...
        All five constraints are normalized by l_total (the total arm length, ~1.1 m)
        ...
        Constraints (all normalized by l_total):
          cup_z / l_total                ground clearance
          (rad_max - slosh) / l_total    spill threshold
          joint_slack / joint_limits[0]  joint limits
          raw_clearance / l_total        obstacle clearance
          (dist - r_link) / l_total      link self-collision
        """
        ...
        # Joint limits.
        m_joint = min(
            float(np.min(self.joint_limits - np.abs(q))) / float(self.joint_limits[0]),
            1.0,
        )
        ...
        for t in np.linspace(0.0, 1.0, 20):
            ...
            sc_min_d = min(
                sc_min_d, float(np.sqrt(pt[0] ** 2 + pt[1] ** 2 + (pt[2] - z_c) ** 2))
            )
        ...
        return np.min(
            np.stack(
                [
                    ...
                    (self.slosh_rad_max - slosh_disp)
                    / (10.0 * self.slosh_rad_max),  # max = 0.1
    ...
    def hamiltonian(self, state, dvds):
        ...
        scale = torch.as_tensor(
            self.state_scale, dtype=torch.float32, device=state.device
        )
        ...
        Jd[:, 0, 0] = (
            (l3 * c1 * s2 * s3 - l3 * c1 * c2 * c3 - l2 * c1 * c2) * td1
            + (l3 * s1 * s2 * c3 + l3 * s1 * c2 * s3 + l2 * s1 * s2) * td2
            + (s1 * c2 * s3 + s1 * s2 * c3) * l3 * td3
        )
        Jd[:, 0, 1] = (
            (l3 * s1 * s2 * c3 + l3 * s1 * c2 * s3 + l2 * s1 * s2) * td1
            + (l3 * c1 * s2 * s3 - l3 * c1 * c2 * c3 - l2 * c1 * c2) * td2
            + (c1 * s2 * s3 - c1 * c2 * c3) * l3 * td3
        )
        Jd[:, 0, 2] = (
            (s1 * c2 * s3 + s1 * s2 * c3) * l3 * td1
            + (c1 * s2 * s3 - c1 * c2 * c3) * l3 * td2
            + (c1 * s2 * s3 - c1 * c2 * c3) * l3 * td3
        )
        Jd[:, 1, 0] = (
            (l3 * s1 * s2 * s3 - l3 * c2 * c3 * s1 - l2 * c2 * s1) * td1
            - (l3 * s2 * c3 * c1 + l3 * c1 * c2 * s3 + l2 * c1 * s2) * td2
            - (c1 * s2 * c3 + c1 * c2 * s3) * l3 * td3
        )
        Jd[:, 1, 1] = (
            -(l3 * s2 * c3 * c1 + l3 * c1 * c2 * s3 + l2 * c1 * s2) * td1
            + (l3 * s1 * s2 * s3 - l3 * c2 * c3 * s1 - l2 * s1 * c2) * td2
            + (s2 * s3 * s1 - s1 * c2 * c3) * l3 * td3
        )
        Jd[:, 1, 2] = (
            -(c1 * c2 * s3 + c1 * s2 * c3) * l3 * td1
            + (s1 * s2 * s3 - s1 * c2 * c3) * l3 * td2
            + (s1 * s2 * s3 - s1 * c2 * c3) * l3 * td3
        )
        Jd[:, 2, 0] = 0.0
        Jd[:, 2, 1] = (
            -(l2 * s2 + l3 * c2 * s3 + l3 * s2 * c3) * td2
            - (s2 * c3 + c2 * s3) * l3 * td3
        )
