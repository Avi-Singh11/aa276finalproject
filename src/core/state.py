...
def split_state(x: np.ndarray):
    """Split state into arm and slosh parts."""
...
def join_state(arm: np.ndarray, slosh: np.ndarray):
    """Combine arm and slosh into a single state vector."""
    ...
    if arm.shape[0] != ARM_STATE_DIM:
        raise ValueError(f"Expected {ARM_STATE_DIM}D arm state, got {arm.shape[0]}D")
    if slosh.shape[0] != SLOSH_STATE_DIM:
        raise ValueError(f"Expected {SLOSH_STATE_DIM}D slosh state, got {slosh.shape[0]}D")
